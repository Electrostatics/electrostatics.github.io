...
title: Treecode-Accelerated Boundary Integral (TABI) Poisson-Boltzmann Solver
prev_section:
next_section:
permalink:
---
...
where $G$ is the screened Coulomb potential kernel, $\textbf{x}_i, \textbf{x}_j$ are the centroids of the triangles, and $q_j$ is the charge at $\textbf{x}_j$. The particles (centroids of the triangles) are divided into a hierarchy of clusters having a tree structure. The treecode replaces the O(N^2) particle-particle interactions by O(NlogN) particle-cluster interactions and TABI utilizes this feature efficiently. For more details, refer to <a href="http://www.sciencedirect.com/science/article/pii/S0021999113002404">Geng (2013)</a>.

### APBS Implementation

Currently, TABI on APBS is able to <a href="#electrostatics">compute electrostatic energies, generate surface potentials and calculate pKa</a>. New applications of TABI will be released after the completion of each module.
...
Users can follow <a href="http://www.poissonboltzmann.org/examples/binding_energies/">binding energy section</a> to use TABI-PB method to calculate free energy on <code>../test_proteins/1d30.pqr</code>, <code>../test_proteins/1d30_monomer1.pqr</code> and <code>../test_proteins/1d30_monomer2.pqr</code>. In the <code>binding_energy</code> folder, we provide:
...
After calculating and collecting these free energies by <code>$ apbs foo.in</code>,
